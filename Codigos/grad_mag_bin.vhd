...
architecture rtl of grad_mag_bin is
  ------------------------------------------------------------------------------
  -- TAN/LUT CONFIG
  --  - Recomendo TAN_SCALE = 4096 (2^12) e TAN_SCALE_BITS >= 16
  --  - LUT para ângulos: 20°, 40°, 60°, 80° (9 bins em 0..180)
  --  - Valores aproximados: tan(20)=0.36397, tan(40)=0.83910,
  --                         tan(60)=1.73205, tan(80)=5.67128
  --    TAN_x = round(tan(x)*TAN_SCALE)
  ------------------------------------------------------------------------------
  constant TAN_SCALE_BITS : integer := 16;
  constant TAN_SCALE      : unsigned(TAN_SCALE_BITS-1 downto 0) := to_unsigned(4096, TAN_SCALE_BITS);

  constant TAN_20 : unsigned(TAN_SCALE_BITS-1 downto 0) := to_unsigned(1491,  TAN_SCALE_BITS); -- round(0.36397*4096)
  constant TAN_40 : unsigned(TAN_SCALE_BITS-1 downto 0) := to_unsigned(3439,  TAN_SCALE_BITS); -- round(0.83910*4096)
  constant TAN_60 : unsigned(TAN_SCALE_BITS-1 downto 0) := to_unsigned(7094,  TAN_SCALE_BITS); -- round(1.73205*4096)
  constant TAN_80 : unsigned(TAN_SCALE_BITS-1 downto 0) := to_unsigned(23249, TAN_SCALE_BITS); -- round(5.67128*4096)
      ...
      if rst='1' then
        ...
        if in_ready_i='1' then
          ...
          if in_valid='1' then
            ...
            -- produtos (cross-multiply) explícitos (sem resize implícito no *)
            prod_lhs := resize(gy_ext, MULW_LHS) * resize(TAN_SCALE, MULW_LHS);
            rhs20    := resize(gx_ext, MULW_RHS) * resize(TAN_20,  MULW_RHS);
            rhs40    := resize(gx_ext, MULW_RHS) * resize(TAN_40,  MULW_RHS);
            rhs60    := resize(gx_ext, MULW_RHS) * resize(TAN_60,  MULW_RHS);
            rhs80    := resize(gx_ext, MULW_RHS) * resize(TAN_80,  MULW_RHS);
